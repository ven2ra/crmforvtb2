function History(){
  const {Select,Badge,Avatar}=window.CRMDesignSystem_14b93b;
  const rows=[
  ...
  return (<div style={{padding:32,display:'flex',flexDirection:'column',gap:20}}>
    ...
    <div style={{background:'var(--bg-surface)',border:'1px solid var(--border-subtle)',borderRadius:'var(--radius-xl)',overflow:'hidden'}}>
      ...
      {rows.map(r=>(<div key={r.id} className="row-hover" style={{display:'grid',gridTemplateColumns:'80px 2fr 1.2fr 1fr 110px 90px 90px',gap:12,padding:'14px 20px',alignItems:'center',borderBottom:'1px solid var(--border-subtle)',font:'var(--text-body-sm)',color:'var(--text-primary)'}}>
