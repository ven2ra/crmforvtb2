function Sidebar({active,onNav}){
  const {NavItem,Icon,Avatar}=window.CRMDesignSystem_14b93b;
  const items=[{key:'home',icon:'home',label:'Главная'},{key:'tickets',icon:'inbox',label:'Обращения'},{key:'calls',icon:'phone',label:'Звонки'},{key:'chats',icon:'message',label:'Чаты'},{key:'history',icon:'clock',label:'История'}];
  return (<div style={{width:240,background:'var(--bg-surface)',borderRight:'1px solid var(--border-subtle)',display:'flex',flexDirection:'column',padding:'20px 14px',gap:4,height:'100%',flexShrink:0}}>
    ...
    {items.map(it=>(<NavItem key={it.key} icon={<Icon name={it.icon}/>} label={it.label} active={active===it.key} onClick={()=>onNav(it.key)}/>))}
    <div style={{flex:1}}/>
